...
export { createPilotController, createGroundModel, createSpacecraftModel, createRoadModel, createBoatModel, createBirdModel, createFishModel, createBikeModel, createGrappleModel, carryMomentum, scaleProfileSpeeds, NO_WATER, ATV_PROFILE, CRAFT_PROFILE, ROAD_PROFILE, BOAT_PROFILE, BIRD_PROFILE, FISH_PROFILE, BIKE_PROFILE, GRAPPLE_PROFILE, HELI_PROFILE, PLANE_PROFILE } from './src/pilot.js';   // A-FEEL: createRoadModel = a car that corners on a street grid (curvature-limited steering, R grows with v squared). A-FISH: createFishModel = neutral buoyancy + pivot-in-place turning + a ballistic breach. A-MOTO: createBikeModel = the two-phase dirtbike (terrain-follow + ballistic air with pitch control + a landing verdict). A-DRIVE: HELI_PROFILE (lifted out of metropolis) and PLANE_PROFILE are PROFILES on the spacecraft and the bird — a helicopter and a fixed-wing plane needed numbers, not integrators
export { createVehicleGlbMesh, createBoxVehicleMesh } from './src/createVehicleMesh.js';   // A-DRIVE: ONE loader for every modelled vehicle — articulated parts (rotors, propellers, wheels) arrive as a DATA table, so a project adds a craft with a roster row plus a GLB
